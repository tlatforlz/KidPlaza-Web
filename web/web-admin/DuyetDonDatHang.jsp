<%-- 
    Document   : DuyetDonDatHang
    Created on : May 7, 2017, 1:28:33 PM
    Author     : tranl
--%>
        ...
        <div id="wrapper">
            ...
            <div class="graphs">
                ...
                <div class="xs">
                    ...
                    <p> Tình trạng thanh toán :  <%=status%></p>
                    <p> Địa chỉ giao hàng :  <%=ddh.getDiaChiGiaoHang()%></p>

                    <input style="display:none" id="tinhtrang" value="<%=ddh.getTinhTrangGiao()%>">
                    <input style="display:none" id="dahuy" value="<%=ddh.getDaHuy()%>">
                    <script>
            $(document).ready(function () {
                var gt = $("#tinhtrang").val();
                var dh = $("#dahuy").val();
                if (dh === 1) {

                }
                if (gt === "0") {
                    $("#button-duyet").text("Duyệt đơn hàng");
                    $("#tinhtranggiao").text("Đang đợi duyệt đơn hàng");
                }
                if (gt === "1") {
                    $("#button-duyet").text("Chuẩn bị đơn hàng");
                    $("#tinhtranggiao").text("Đã xác nhận đơn hàng");
                }
                if (gt === "2") {
                    $("#button-duyet").text("Đang giao hàng");
                    $("#tinhtranggiao").text("Đơn hàng đang chuẩn bị");
                }
                if (gt === "3") {
                    $("#button-duyet").text("Đã giao thành công");
                    $("#tinhtranggiao").text("Đang giao hàng");

                }
                if (gt === "4") {
                    $("#thietlap").css("display", "none");

                    $("#tinhtranggiao").text("Đã giao thành công");
                    $("#btn-huy").css("display", "none");
                }
                if (gt === "5") {
                    $("#thietlap").css("display", "none");
                    $("#tinhtranggiao").text("Đã hoàn trả");
                    $("#btn-huy").css("display", "none");
                }
                if (gt === "6") {
                    $("#tinhtranggiao").text("Yêu cầu hoàn trả");
                    $("#button-duyet").text("Chấp nhận hoàn trả");
                    $("#btn-huy").css("display", "none");
                }
            });
                    </script> 
                    <span style="display:inline"> Tình trạng giao hàng : 
                        <p style="display:inline" id="tinhtranggiao">  </p>
                    </span>
                    <br>
                    <span id="thietlap">
                        Thiết lập tình trạng giao hàng : 
                        <button class="btn btn-default"><a href="ThietLapGiaoHang?MaDonDatHang=<%=ddh.getMaDonDatHang()%>&TinhTrang=<%=ddh.getTinhTrangGiao()%>" id="button-duyet">Duyệt đơn hàng</a></button>
                        <button id="btn-huy" class="btn btn-primary"><a style="color:black" href="HuyDonDatHang?MaDonDatHang=<%=ddh.getMaDonDatHang()%>">Hủy đơn hàng</a></button>
                    </span>
                    <hr>
                    ...
                    <button class="btn btn-primary"><a style="color:black" href="DuyetDonDatHang">Quay lại</a></button>
                </div>
            </div>
        </div>
    </div>
